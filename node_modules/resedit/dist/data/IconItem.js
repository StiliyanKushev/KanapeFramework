...
function calcMaskSize(width, height) {
    var actualWidth = functions_1.roundUp(Math.abs(width) / 8, 4);
...
var IconItem = /** @class */ (function () {
    function IconItem(width, height, bin, byteOffset, byteLength) {
        ...
        var sizeImage = functions_1.readUint32WithLastOffset(view, 20, headerSize);
        var bi = {
            width: functions_1.readInt32WithLastOffset(view, 4, headerSize),
            height: functions_1.readInt32WithLastOffset(view, 8, headerSize),
            planes: functions_1.readUint16WithLastOffset(view, 12, headerSize),
            bitCount: functions_1.readUint16WithLastOffset(view, 14, headerSize),
            compression: functions_1.readUint32WithLastOffset(view, 16, headerSize),
            sizeImage: sizeImage,
            xPelsPerMeter: functions_1.readInt32WithLastOffset(view, 24, headerSize),
            yPelsPerMeter: functions_1.readInt32WithLastOffset(view, 28, headerSize),
            colorUsed: functions_1.readUint32WithLastOffset(view, 32, headerSize),
            colorImportant: functions_1.readUint32WithLastOffset(view, 36, headerSize),
            colors: [],
        };
        var offset = 40;
        ...
        for (var i = 0; i < colors; ++i) {
            bi.colors.push({
                b: functions_1.readUint8WithLastOffset(view, offset, totalSize),
                g: functions_1.readUint8WithLastOffset(view, offset + 1, totalSize),
                r: functions_1.readUint8WithLastOffset(view, offset + 2, totalSize),
            });
            offset += 4;
        }
        this.width = width;
        this.height = height;
        this.bitmapInfo = bi;
        var absWidthRound = functions_1.roundUp(Math.abs(bi.width), 8);
        var absActualHeight = Math.abs(bi.height) / 2;
        var size = sizeImage || (bi.bitCount * absWidthRound * absActualHeight) / 8;
        this._pixels = functions_1.allocatePartialBinary(view, offset, size);
        offset += size;
    ...
    IconItem.prototype.generate = function () {
        ...
        var absWidthRound = functions_1.roundUp(absWidth, 8);
        var absActualHeight = Math.abs(bi.height) / 2;
        ...
        functions_1.copyBuffer(bin, offset, this.pixels, 0, actualSizeImage);
        functions_1.copyBuffer(bin, offset + actualSizeImage, this.masks, 0, sizeMask);
        return bin;
    };
    return IconItem;
}());
